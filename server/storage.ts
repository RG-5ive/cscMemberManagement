...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByEmailAndRole(
    ...
    if (user) {
      console.log(
        `Found user with matching email and role: ID ${user.id}, username ${user.username}`
      );
    } else {
  ...
  async createUser(userData: InsertUser | any): Promise<User> {
  ...
  async deleteMessageGroup(groupId: number): Promise<void> {
    ...
    await db
      .delete(messageGroupMembers)
      .where(eq(messageGroupMembers.groupId, groupId));

    // Then delete messages sent to the group
    await db.delete(messages).where(eq(messages.toGroupId, groupId));

    // Finally delete the group itself
    await db.delete(messageGroups).where(eq(messageGroups.id, groupId));
  }
  ...
  // Survey operations
  async createSurvey(
    survey: Omit<Survey, "id" | "createdAt">
  ): Promise<Survey> {
  ...
  async registerForWorkshop(
    registration: Omit<WorkshopRegistration, "id" | "registeredAt">
  ...
  async getMemberData(
    ...
    firstName: string,
    lastName: string
  ): Promise<{ memberLevel: string | null } | null> {
