...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/discord/bot/start", requireAuth, async (req, res) => {
    try {
      ...
      res.json({
        success: started,
        message: started ? "Bot started successfully" : "Failed to start bot",
      });
    } catch (error) {
  ...
  app.get("/api/discord/server-info", requireAuth, async (req, res) => {
    try {
      ...
      if (!guildId) {
        return res
          .status(400)
          .json({ error: "Discord guild ID not configured" });
      }
  ...
  app.post("/api/discord/channels", requireAuth, async (req, res) => {
    try {
      ...
      const channel = await bot.createChannelFromWeb(
        guildId,
        name,
        type,
        description
  ...
  app.post("/api/discord/announcements", requireAuth, async (req, res) => {
    try {
      ...
      const targetChannelId =
        channelId || process.env.DISCORD_ANNOUNCEMENT_CHANNEL_ID;
      if (!targetChannelId) {
        return res
          .status(400)
          .json({ error: "No announcement channel configured" });
      }

      const success = await bot.sendAnnouncementFromWeb(
        guildId,
        targetChannelId,
        title,
        message
  ...
  app.post("/api/discord/roles", requireAuth, async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: "Role creation via API not yet implemented",
      });
    } catch (error) {
  ...
  app.patch("/api/message-groups/:id", requireAuth, async (req, res) => {
    try {
      ...
      const updatedGroup = await storage.updateMessageGroup(
        groupId,
        updateData
      );
  ...
  app.post("/api/message-groups/:id/members", requireAuth, async (req, res) => {
    try {
      ...
      const member = await storage.addMemberToGroup(
        groupId,
        memberId,
        req.user!.id
      );
  ...
  app.delete(
    "/api/message-groups/:groupId/members/:memberId",
    requireAuth,
    async (req, res) => {
      try {
        ...
        if (isNaN(groupId) || isNaN(memberId)) {
          return res
            .status(400)
            .json({ error: "Invalid group ID or member ID" });
        }
  ...
  app.post("/api/message-groups/:id/send", requireAuth, async (req, res) => {
    try {
      ...
      if (isNaN(groupId) || !content) {
        return res
          .status(400)
          .json({ error: "Invalid group ID or message content" });
      }

      const message = await storage.sendMessageToGroup(
        req.user!.id,
        groupId,
        content
      );
  ...
  const filterMemberData = (member: any, canAccessDemographics: boolean) => {
    ...
    const { gender, lgbtq2Status, bipocStatus, ethnicity, ...filteredMember } =
      member;
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    app.get("/api/dev/verification-code", (req, res) => {
      ...
      if (verificationEmails.length === 0) {
        return res
          .status(404)
          .json({ error: "No verification emails found for this address" });
      }
  ...
  app.post("/api/verify/member", async (req, res) => {
    try {
      // Validate input
      const input = verificationInputSchema
      ...
      if (!input.success) {
        return res
          .status(400)
          .json({ error: "Invalid input", details: input.error.format() });
      }

      const { email, firstName, lastName } = input.data;

      // Check if member exists in database and get their data
      const memberExists = await storage.checkMemberExists(
        email,
        firstName,
        lastName
      );
      ...
      const memberData = await storage.getMemberData(
        email,
        firstName,
        lastName
      );
  ...
  app.post("/api/register", async (req, res) => {
    try {
      ...
      console.log(
        "- password:",
        req.body.password ? "***provided***" : "MISSING",
        typeof req.body.password
      );
      console.log(
        "- memberLevel:",
        req.body.memberLevel,
        typeof req.body.memberLevel
      );
      ...
      const { email, firstName, lastName, password, memberLevel, role } =
        validation.data;
  ...
  app.post("/api/user/email-change-request", requireAuth, async (req, res) => {
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        log(
          `Email change verification code for ${newEmail}: ${verificationCode}`,
          "mail"
        );
      }
  ...
  app.post("/api/user/resend-verification", requireAuth, async (req, res) => {
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        log(
          `Resent verification code for ${latestVerification.email}: ${latestVerification.code}`,
          "mail"
        );
      }
  ...
  app.patch("/api/user/:id", requireAuth, async (req, res) => {
    try {
      ...
      console.log(
        `Profile update attempt for user ${userId} by ${req.user?.id}`
      );

      // Verify user is updating their own profile or is an admin
      if (userId !== req.user?.id && req.user?.role !== "admin") {
        console.log(
          "Forbidden: User attempting to update another user profile"
        );
        return res
          .status(403)
      ...
      if (req.body.hasCompletedOnboarding === true && updatedUser.email) {
        try {
          console.log(
            `User ${userId} completed onboarding, updating member portal access for email: ${updatedUser.email}`
          );
          ...
          if (result.length > 0) {
            console.log(
              `Updated portal access for member ID ${result[0].id} (${updatedUser.email})`
            );
          } else {
            console.log(
              `No member record found for email ${updatedUser.email} - user may not be in members table`
            );
          }
        } catch (memberUpdateError) {
          console.error(
            "Error updating member portal access:",
            memberUpdateError
          );
  ...
  app.post("/api/message-groups", requireAuth, async (req, res) => {
    try {
      if (!req.user || req.user.role !== "admin") {
        return res
          .status(403)
          .json({ error: "Only administrators can create message groups" });
      }
  ...
  app.get("/api/message-groups", requireAuth, async (req, res) => {
    try {
      if (!req.user || req.user.role !== "admin") {
        return res
          .status(403)
          .json({ error: "Only administrators can view message groups" });
      }
  ...
  app.patch("/api/message-groups/:id", requireAuth, async (req, res) => {
    try {
      if (!req.user || req.user.role !== "admin") {
        return res
          .status(403)
          .json({ error: "Only administrators can update message groups" });
      }
  ...
  app.delete("/api/message-groups/:id", requireAuth, async (req, res) => {
    try {
      if (!req.user || req.user.role !== "admin") {
        return res
          .status(403)
          .json({ error: "Only administrators can delete message groups" });
      }
  ...
  app.post("/api/message-groups/:id/members", requireAuth, async (req, res) => {
    try {
      if (!req.user || req.user.role !== "admin") {
        return res
          .status(403)
          .json({ error: "Only administrators can add members to groups" });
      }
      ...
      try {
        if (!req.user || req.user.role !== "admin") {
          return res.status(403).json({
            error: "Only administrators can remove members from groups",
          });
        }
  ...
  app.get("/api/message-groups/:id/members", requireAuth, async (req, res) => {
    try {
      if (!req.user || req.user.role !== "admin") {
        return res
          .status(403)
          .json({ error: "Only administrators can view group members" });
      }
    ...
    async (req, res) => {
      try {
        if (!req.user || req.user.role !== "admin") {
          return res
            .status(403)
            .json({ error: "Only administrators can send group messages" });
        }
  ...
  app.get("/api/committees", requireAuth, async (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(403).json({
          error: "Access denied. Committee management privileges required.",
        });
      }
  ...
  app.post("/api/committees", requireAuth, async (req, res) => {
    ...
    if (!req.user || req.user.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Only administrators can create committees" });
    }
    ...
    try {
      ...
      if (
        ...
        for (const memberAssignment of membersToAdd) {
          ...
          if (!userId || !roleId) {
            console.warn(
              `Skipping member assignment - missing userId (${userId}) or roleId (${roleId})`
            );
            continue;
          }
          ...
          try {
            ...
            console.error(
              `Error adding member ${userId} to committee:`,
              memberError
            );
  ...
  app.put("/api/committees/:id", requireAuth, async (req, res) => {
    ...
    if (!req.user || req.user.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Only administrators can update committees" });
    }
    ...
    async (req, res) => {
      try {
        ...
        if (
          ...
          const userCommitteeMemberships = await db
            .select()
            .from(committeeMembers)
            .where(
              and(
          ...
          hasPermission = userCommitteeMemberships.some(
            (membership) =>
              membership.committee_roles.name === "Chair" ||
              membership.committee_roles.name === "Co-Chair"
          );
        }
        ...
        if (!hasPermission) {
          return res.status(403).json({
            error:
              "Access denied. You need to be an admin or chair/co-chair of this committee.",
          });
        }
  ...
  app.delete("/api/committees/:id", requireAuth, async (req, res) => {
    ...
    if (!req.user || req.user.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Only administrators can delete committees" });
    }
    ...
    try {
      ...
      const workshopsWithCommittee = await db
        .select()
        .from(workshops)
  ...
  app.get("/api/committee-roles", requireAuth, async (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(403).json({
          error:
            "Access denied. Admin or committee management privileges required.",
        });
      }
  ...
  app.post("/api/committee-roles", requireAuth, async (req, res) => {
    try {
      ...
      const { name, description, canManageCommittee, canManageWorkshops } =
        req.body;
    ...
    try {
      ...
      const [updatedRole] = await db
        ...
        .set({
          ...
          description,
          canManageCommittee:
            canManageCommittee !== undefined
              ? canManageCommittee
              : existingRole.canManageCommittee,
          canManageWorkshops:
            canManageWorkshops !== undefined
              ? canManageWorkshops
              : existingRole.canManageWorkshops,
        })
        .where(eq(committeeRoles.id, roleId))
        .returning();
  ...
  app.delete("/api/committee-roles/:id", requireAuth, async (req, res) => {
    try {
      ...
      const committeeMembersData = await db
        .select()
        .from(committeeMembers)
      ...
      if (committeeMembersData.length > 0) {
        return res.status(400).json({
          error:
            "Cannot delete role as it's currently assigned to committee members",
          message:
            "You must reassign all members using this role before deleting it",
        });
      }
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        console.log(
          `Fetching committee members for committee ${committeeId}, includeInactive=${includeInactive}`
        );
        ...
        const committeesMembersData = await db
          .select({
    ...
    async (req, res) => {
      try {
        ...
        if (isNaN(committeeId) || !userId || !roleId) {
          ...
          return res.status(400).json({
            error:
              "Invalid input. Committee ID, user ID, and role ID are required.",
          });
        }
        ...
        console.log(
          `Committee check: found ${committeeResult.length} committees with ID ${committeeId}`
        );
        ...
        console.log(
          `User check: found ${userResult.length} users with ID ${userId}`
        );
        ...
        console.log(
          `Role check: found ${roleResult.length} roles with ID ${roleId}`
        );
        ...
        console.log(
          `Active membership check: found ${existingMember.length} active memberships`
        );
        ...
        console.log(
          `Previous membership check: found ${previousMembership.length} ended memberships`
        );

        // If active membership exists, return error
        if (existingMember.length > 0) {
          return res.status(400).json({
            error: "User is already an active member of this committee",
          });
        }
        ...
        if (role.canManageCommittee || role.canManageWorkshops) {
          console.log(
            "Role has management permissions, updating user permissions"
          );
          const updateData = {
            canManageCommittees:
              role.canManageCommittee || user.canManageCommittees,
            canManageWorkshops:
              role.canManageWorkshops || user.canManageWorkshops,
          };
        ...
        const newMemberDetails = await db
          .select({
            id: committeeMembers.id,
            startDate: committeeMembers.startDate,
            endDate: committeeMembers.endDate,
        ...
        res.status(500).json({
          error: "Failed to add committee member: " + (error as Error).message,
        });
      }
    }
  );
    ...
    async (req, res) => {
      try {
        ...
        if (isNaN(committeeId) || isNaN(committeeMemberId)) {
          return res
            .status(400)
            .json({ error: "Invalid committee ID or member ID" });
        }
        ...
        const currentRoleResult = await db
          .select()
          .from(committeeRoles)
        ...
        const updatedMemberDetails = await db
          .select({
            id: committeeMembers.id,
            startDate: committeeMembers.startDate,
            endDate: committeeMembers.endDate,
    ...
    async (req, res) => {
      try {
        ...
        if (req.user?.role !== "admin") {
          return res.status(403).json({
            error:
              "Access denied. Admin privileges required to set chair credentials.",
          });
        }
        ...
        if (
          ...
          return res.status(400).json({
            error:
              "Invalid input. Committee ID, user ID, username, password, and role are required.",
          });
        }
        ...
        if (role !== "committee_chair" && role !== "committee_cochair") {
          return res.status(400).json({
            error:
              "Invalid role. Only 'committee_chair' or 'committee_cochair' roles can have credentials set.",
          });
        }
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        console.log(
          `Delete request for committee ${committeeId}, member ${committeeMemberId}`
        );
        ...
        if (committeeMember.endDate !== null) {
          ...
          console.log(
            `Committee member ${committeeMemberId} already has an end date set:`,
            committeeMember.endDate
          );
          return res.status(200).json({
            message: "Member was already removed from this committee",
          });
        }
        ...
        if (role.canManageCommittee || role.canManageWorkshops) {
          ...
          console.log(
            `Found ${otherActiveMembers.length} other active committee memberships for user ${committeeMember.userId}`
          );
          ...
          if (!userData) {
            console.log(
              `User ${committeeMember.userId} not found when updating permissions`
            );
          ...
          if (userData.role === "admin") {
            console.log(
              `User ${committeeMember.userId} is an admin, preserving all admin privileges`
            );
            ...
            if (otherActiveMembers.length > 0) {
              ...
              if (!hasOtherManagementRoles) {
                ...
                if (
                  userData.role === "committee_chair" ||
                  userData.role === "committee_cochair" ||
                ...
                console.log(
                  `Reset committee permissions for user ${committeeMember.userId} - no other management roles`
                );
              } else {
                console.log(
                  `User ${committeeMember.userId} still has other management roles, keeping permissions`
                );
              }
            } else {
              ...
              console.log(
                `Reset permissions for user ${committeeMember.userId} - no other committee memberships`
              );
            }
          }
        }
        ...
        res
          .status(200)
          .json({ message: "Committee member removed successfully" });
      } catch (error) {
  ...
  app.post("/api/membership-pricing-rules", requireAuth, async (req, res) => {
    if (!req.user || req.user.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Only administrators can manage pricing rules" });
    }
  ...
  app.patch(
    "/api/membership-pricing-rules/:id",
    requireAuth,
    async (req, res) => {
      if (!req.user || req.user.role !== "admin") {
        return res
      ...
      try {
        ...
        if (percentagePaid === undefined) {
          return res
            .status(400)
            .json({ error: "Missing percentagePaid field" });
        }
  ...
  app.post(
    "/api/membership-pricing-rules/seed",
    requireAuth,
    async (req, res) => {
      if (!req.user || req.user.role !== "admin") {
        return res
          .status(403)
          .json({ error: "Only administrators can seed pricing rules" });
      }
      ...
      try {
        ...
        for (const rule of defaultRules) {
          ...
          const [existing] = await db
            ...
            .from(membershipPricingRules)
            .where(
              eq(membershipPricingRules.membershipLevel, rule.membershipLevel)
            );
  ...
  app.post("/api/workshops", requireAuth, async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        return res
          .status(400)
          .json({ error: "Invalid date format. Please use a valid date." });
      }

      // Check if specified committee exists
      if (committeeId) {
        const [committeeExists] = await db
          .select()
          .from(committees)
          .where(eq(committees.id, committeeId));
        ...
        if (!committeeExists) {
          return res
            .status(400)
            .json({ error: "Specified committee does not exist" });
        }
      }
      ...
      const workshopData = {
        ...
        globalDiscountPercentage: globalDiscountPercentage
          ? Number(globalDiscountPercentage)
          : 0,
  ...
  app.get("/api/workshops", requireAuth, async (req, res) => {
    try {
      ...
      const workshopsWithCommittees = await db
        .select({
          id: workshops.id,
        ...
        .from(workshops)
        .leftJoin(committees, eq(workshops.committeeId, committees.id))
  ...
  app.patch("/api/workshops/:id", requireAuth, async (req, res) => {
    ...
    try {
      ...
      if (globalDiscountPercentage !== undefined) {
        updateData.globalDiscountPercentage = globalDiscountPercentage
          ? Number(globalDiscountPercentage)
          : 0;
      }
  ...
  app.patch("/api/workshops/:id/visibility", requireAuth, async (req, res) => {
    ...
    try {
      ...
      console.log(
        `Workshop ${workshopId} visibility updated by admin ${req.user.id}`
      );
      res.json(updatedWorkshop);
    } catch (error) {
  ...
  app.get("/api/workshops/:id/registrations", requireAuth, async (req, res) => {
    try {
      ...
      if (!req.user || req.user.role !== "admin") {
        return res.status(403).json({
          error: "Only administrators can view workshop registrations",
        });
      }
  ...
  app.post(
    "/api/workshops/:id/add-participant",
    requireAuth,
    async (req, res) => {
      console.log(
        "Add participant API called with params:",
        req.params,
        "and body:",
        req.body
      );
      try {
        ...
        if (!req.user || req.user.role !== "admin") {
          return res
            .status(403)
            .json({ error: "Only administrators can add participants" });
        }
        ...
        if (isNaN(workshopId) || !userId) {
          return res
            .status(400)
            .json({ error: "Invalid workshop ID or user ID" });
        }
        ...
        if (existingRegistration.length > 0) {
          return res
            .status(400)
            .json({ error: "User is already registered for this workshop" });
        }
        ...
        if (currentRegistrations >= workshop.capacity) {
          return res
            .status(400)
            .json({ error: "Workshop is at full capacity" });
        }
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        if (!req.user || req.user.role !== "admin") {
          return res.status(403).json({
            error: "Only administrators can remove workshop registrations",
          });
        }
  ...
  app.post(
    "/api/workshops/registration/:id/approve-payment",
    requireAuth,
    async (req, res) => {
      try {
        ...
        if (!req.user || req.user.role !== "admin") {
          return res
            .status(403)
            .json({ error: "Only administrators can approve payments" });
        }
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (webhookSecret) {
          ...
          console.warn(
            "WARNING: Processing webhook without signature verification. Set STRIPE_WEBHOOK_SECRET for production."
          );
          ...
          case "payment_intent.succeeded": {
            ...
            if (payment) {
              ...
              await db
                ...
                .where(
                  eq(workshopRegistrations.id, payment.workshopRegistrationId!)
                );
              ...
              await db
                ...
                .where(
                  eq(
                    invoices.workshopRegistrationId,
  ...
  app.get("/api/users/search", requireAuth, async (req, res) => {
    try {
      ...
      if (
        ...
        console.log(
          "Unauthorized user attempted to search users:",
          req.user?.role
        );
        return res.status(403).json({ error: "Unauthorized" });
      }
  ...
  app.post(
    "/api/workshops/:workshopId/registrations/:registrationId/approve",
    requireAuth,
    async (req, res) => {
      try {
        ...
        if (!req.user || req.user.role !== "admin") {
          return res.status(403).json({
            error: "Only administrators can approve workshop registrations",
          });
        }
  ...
  app.post(
    "/api/workshops/:workshopId/registrations/:registrationId/confirm-payment",
    requireAuth,
    async (req, res) => {
      try {
        ...
        if (!req.user || req.user.role !== "admin") {
          return res.status(403).json({
            error: "Only administrators can confirm workshop payments",
          });
        }
        ...
        console.error(
          "Error confirming payment for workshop registration:",
          error
        );
        res.status(500).json({ error: (error as Error).message });
      }
  ...
  app.post(
    "/api/workshops/:workshopId/registrations/:registrationId/notes",
    requireAuth,
    async (req, res) => {
      try {
        ...
        if (!req.user || req.user.role !== "admin") {
          return res.status(403).json({
            error: "Only administrators can add notes to registrations",
          });
        }
      ...
      if (relatedPayments.length > 0) {
        await db.delete(auditLogs).where(
          and(
            ...
            sql`${auditLogs.entityId} IN (${sql.join(
              relatedPayments.map((p) => sql`${p.id}`),
              sql`, `
            )})`
          )
        );
  ...
  app.get("/api/members/category/:category", requireAuth, async (req, res) => {
    try {
      ...
      const categoryMembers = await db
        .select()
        .from(members)
  ...
  app.get("/api/members/search", requireAuth, async (req, res) => {
    ...
    try {
      ...
      if (
        ...
        console.log(
          "Unauthorized user attempted to search members:",
          req.user?.role
        );
        return res.status(403).json({
          error: "Access denied",
          message:
            "You need committee management privileges to search the member database",
        });
      }
      ...
      if (queryWords.length === 2) {
        ...
        if (exactResults.length > 0) {
          ...
          const beginningResults = await db
            .select()
            .from(members)
            .where(
              or(
        ...
        if (queryWords.length > 2) {
          ...
          const multiWordResults = await db
            .select()
            .from(members)
      ...
      const memberEmails = searchResult
      ...
      const membersWithUserInfo = searchResult.map((member) => {
        const userAccount = member.email
          ? usersByEmail[member.email.toLowerCase()]
          : null;
        const memberData = {
          ...member,
  ...
  app.post("/api/members", requireAuth, async (req, res) => {
    try {
      ...
      if (!memberData.memberNumber) {
        ...
        const results = await db.select().from(members).limit(1);
  ...
  app.post("/api/members/continue-import", requireAuth, async (req, res) => {
    try {
      ...
      const importProcess = exec(
        "npx tsx continue-import.ts",
  ...
  app.post("/api/users/admin", requireAuth, async (req, res) => {
    try {
      ...
      console.log(
        `New admin user created: ${username} (${email}) with role: ${userRole}`
      );
  ...
  app.get("/api/users/me/committee-roles", requireAuth, async (req, res) => {
    try {
      ...
      console.log(
        `Found ${memberships.length} committee memberships:`,
        memberships
      );
  ...
  app.post("/api/workshops/:id/add-member", requireAuth, async (req, res) => {
    try {
      ...
      if (isNaN(workshopId) || !memberId) {
        return res
          .status(400)
          .json({ error: "Workshop ID and member ID are required" });
      }
      ...
      if (!existingUser && member.email) {
        ...
        const tempUsername =
          member.email.split("@")[0] + "-" + Math.floor(Math.random() * 10000);
        ...
        console.log(
          `Created temporary user account for member #${member.memberNumber} (${member.firstName} ${member.lastName}) with ID ${userId}`
        );
      }
      ...
      if (!userId) {
        return res.status(400).json({
          ...
          message:
            "This member has no email address and cannot be registered for the workshop",
        });
      }
      ...
      if (existingRegistration) {
        return res
          .status(400)
          .json({ error: "Member is already registered for this workshop" });
      }
  ...
  app.post("/api/committees/:id/messages", requireAuth, async (req, res) => {
    try {
      ...
      if (userMembership.length === 0) {
        return res
          .status(403)
          .json({ error: "You are not a member of this committee" });
      }
  ...
  app.post(
    "/api/committees/:id/meetings/instant",
    requireAuth,
    async (req, res) => {
      try {
        const committeeId = parseInt(req.params.id);
        ...
        if (userMembership.length === 0) {
          return res
            .status(403)
            .json({ error: "Only chairs and co-chairs can create meetings" });
        }

        // For now, return a mock Zoom meeting URL
        // In production, this would integrate with Zoom API
        const meetingUrl = `https://zoom.us/j/${Math.floor(
  ...
  app.post(
    "/api/committees/:id/meetings/schedule",
    requireAuth,
    async (req, res) => {
      try {
        const committeeId = parseInt(req.params.id);
        ...
        if (userMembership.length === 0) {
          return res
            .status(403)
            .json({ error: "Only chairs and co-chairs can schedule meetings" });
        }
        ...
        res.json({
          ...
          meeting: {
            ...
            topic:
              topic ||
              `Committee Meeting - ${new Date(startTime).toLocaleDateString()}`,
            startTime: startTime,
  ...
  app.post(
    "/api/committees/:id/calendar/create",
    requireAuth,
    async (req, res) => {
      try {
        const committeeId = parseInt(req.params.id);
        ...
        if (userMembership.length === 0) {
          return res
            .status(403)
            .json({ error: "Only chairs and co-chairs can create calendars" });
        }
        ...
        const calendarId = await googleCalendarService.createCommitteeCalendar(
          committee[0].name
        );
  ...
  app.post(
    "/api/committees/:id/calendar/events",
    requireAuth,
    async (req, res) => {
      try {
        const committeeId = parseInt(req.params.id);
        ...
        if (userMembership.length === 0) {
          return res
            .status(403)
            .json({ error: "Only chairs and co-chairs can create events" });
        }
    ...
    async (req, res) => {
      try {
        ...
        if (userMembership.length === 0) {
          return res
            .status(403)
            .json({ error: "Access denied to this committee" });
        }
  ...
  app.post("/api/calendar-events", requireAuth, async (req, res) => {
    try {
      ...
      if (
        ...
        return res.status(403).json({
          error:
            "Only administrators and committee chairs can create calendar events",
        });
      }
    ...
    async (req, res) => {
      try {
        if (!req.user || req.user.role !== "admin") {
          return res
            .status(403)
            .json({ error: "Only administrators can update event visibility" });
        }
  ...
  app.post(
    "/api/demographic-change-requests",
    requireAuth,
    async (req, res) => {
      try {
        ...
        const currentMember = await db
          .select()
          .from(members)
          .where(eq(members.id, requestData.memberId))
          .limit(1);
        ...
        const validatedData =
          insertDemographicChangeRequestSchema.parse(requestData);

        const request = await db
          .insert(demographicChangeRequests)
        ...
        res
          .status(500)
          .json({ error: "Failed to create demographic change request" });
      }
    }
  );

  app.get(
    "/api/demographic-change-requests/pending",
    requireAuth,
    async (req, res) => {
      try {
        ...
        console.error(
          "Error fetching pending demographic change requests:",
          error
        );
  ...
  app.get(
    "/api/demographic-change-requests/user/:userId",
    requireAuth,
    async (req, res) => {
      try {
        ...
        if (
          req.user!.id !== userId &&
          !req.user!.username.includes("admin@csc.ca") &&
          !req.user!.username.includes("div.chair@csc.ca")
        ) {
        ...
        console.error(
          "Error fetching user demographic change requests:",
          error
        );
  ...
  app.patch(
    "/api/demographic-change-requests/:id",
    requireAuth,
    async (req, res) => {
      try {
        ...
        const updatedRequest = await db
          .update(demographicChangeRequests)
          .set({
            status,
            reviewNotes,
        ...
        if (status === "approved") {
          const requestedChanges = originalRequest[0]
