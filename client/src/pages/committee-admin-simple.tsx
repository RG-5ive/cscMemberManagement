...
export default function CommitteeAdminSimple() {
  ...
  const targetCommitteeId = new URLSearchParams(window.location.search).get(
    "committee"
  )
    ? parseInt(new URLSearchParams(window.location.search).get("committee")!)
    : null;
  ...
  const [calendarEvents, setCalendarEvents] = useState<any>([
    {
      ...
      description:
        "Regular monthly meeting to discuss ongoing projects and initiatives",
      date: "2025-06-15",
      time: "14:00",
      duration: "2 hours",
      attendees: 8,
  ...
  const selectedCommittee = Array.isArray(allCommittees)
    ? allCommittees.find((c: any) => c.id === selectedCommitteeId)
    : null;
  const userRole = Array.isArray(userCommitteeRoles)
    ? userCommitteeRoles.find(
        (role: any) => role.committeeId === selectedCommitteeId
      )
    : null;
        ...
        {selectedCommittee && (
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <Badge
                variant={
                  userRole?.role?.name === "Chair" ? "default" : "secondary"
                }
              >
      ...
      {!selectedCommitteeId ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Please select a committee to manage from the committee selection
              page.
            </CardDescription>
          </CardHeader>
          <CardContent>
